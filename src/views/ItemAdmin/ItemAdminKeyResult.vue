...
export default {
  ...
  methods: {
    ...
    apiCurl: (keyResult) => {
      return `curl -X POST -H '"okr-team-secret": <YOUR SECRET>' -H '"x-api-key": <YOUR API-KEY>' -H 'Content-Type: application/json' -d { "progress": <VALUE> }  ${process.env.VUE_APP_API_GATEWAY_URL}/keyres/${keyResult.id}`;
    },
  },
};
</script>
