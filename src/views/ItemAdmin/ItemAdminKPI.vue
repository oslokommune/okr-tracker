    ...
    <validation-observer v-slot="{ handleSubmit }">
      <form :id="`kpi_${kpi.id}`" @submit.prevent="handleSubmit(save.bind(null, kpi))">
        <label class="form-group">
          ...
          <input v-model="kpi.type" class="form__field" type="text" disabled="disabled" @input="edit"/>
        </label>
        ...
        <label class="form-group">
          ...
          <textarea v-model="kpi.description" class="form__field" rows="4" @input="edit"/>
        </label>
        ...
        <div class="toggle__container">
          ...
          <label class="toggle">
            <input v-model="kpi.api" class="toggle__input" type="checkbox" @input="edit"/>
        ...
        <label v-if="kpi.api" class="form-group">
          ...
          <input :value="apiCurl(kpi)" type="text" disabled class="form__field" @input="edit"/>
        </label>

        <div class="button-row">
          <button class="btn btn--primary" :form="`kpi_${kpi.id}`" :disabled="!changes">{{ $t('btn.saveChanges') }}</button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      apiCurl: (kpi) => {
        return `curl -X POST -H "okr-team-secret: <YOUR SECRET>" -H "x-api-key: <YOUR API-KEY>" -H "Content-Type: application/json" -d '{ "progress": <VALUE> }' ${import.meta.env.VITE_API_GATEWAY_URL}/kpi/${kpi.id}`;
      },
    },
  };
</script>
