<template>
  <div class="container content">
    ...
    <div class="narrow">
      <label class="form-field" :class="{ 'form-field--error': $v.product.$error }">
        <span class="form-label">Produktnavn</span>
        <input type="text" v-model.trim="$v.product.$model" required />
      </label>
      <div class="form-field--error" v-if="$v.product.$error">Må være minimum 4 bokstaver</div>
      <label class="form-field" :class="{ 'form-field--error': $v.mission_statement.$error }">
        <span class="form-label">Mission statement</span>
        <textarea v-model.trim="$v.mission_statement.$model"></textarea>
      </label>
      <div class="form-field--error" v-if="$v.mission_statement.$error">Må være minimum 4 bokstaver</div>

      <span class="form-label" :class="{ 'form-field--error': $v.department.$error }">Avdeling</span>
      <v-select
        class="form-field objective__select"
        :class="{ 'form-field--error': $v.department.$error }"
        v-model="$v.department.$model"
        label="id"
        :options="departments"
      ></v-select>
      <div class="form-field--error" v-if="$v.department.$error">Avdeling må være valgt</div>
