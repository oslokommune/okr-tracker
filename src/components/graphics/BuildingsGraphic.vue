<template>
  <div class="building-wrapper">
    <div class="bird" :style="{ left: '90%', top: `${birdY}px` }" @click="shoo">
      <div class="bird-body">
        <div class="bird-wing-left" :class="{ flapping: flying }"></div>
        <div class="bird-wing-right" :class="{ flapping: flying }"></div>
      </div>
    </div>
    <svg
...
export default {
  ...
  methods: {
    shoo() {
      this.flying = true;
      this.intervalId = setInterval(this.iter, 5);
    },

    iter() {
      this.birdY -= 1;
      if (Math.abs(this.birdY) > document.querySelector('.page').scrollHeight) {
...
.bird-wing-left {
  position: absolute;
  top: calc(var(--bird-size) / 2 - var(--bird-wing-height) / 2);
  left: calc(var(--bird-size) * -1);
  width: var(--bird-size);
  height: var(--bird-wing-height);
  background: var(--color-white);
  transform: rotate(-20deg);
  transform-origin: right;

  &.flapping {
    animation: 0.2s ease-in-out 0s alternate infinite flap;
  }
}
...
.bird-wing-right {
  ...
  &.flapping {
    animation: 0.2s ease-in-out 0s alternate-reverse infinite flap;
  }
}
</style>
