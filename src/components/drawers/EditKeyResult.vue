<template>
 <div class="editKeyResult">
   ...
   <h1 class="heading">{{ keyResult.id? $t('admin.keyResult.change') : $t('admin.keyResult.new') }}</h1>
   <archived-restore v-if="keyResult && keyResult.archived" :restore="restore" :objectType="$t('archived.keyResult')"/>
   <form-section>
     <div v-if="step === 1">
       <form-component
     ...
     <template #actions="{ handleSubmit, submitDisabled }" v-if="!keyResult.archived">
       <div v-if="step===1">
         <btn-cancel :disabled="loading" @click="TOGGLE_DRAWER({show: false})" />
         <btn-save :label="$t('btn.continue')"
                   :disabled="!changed || loading"
                   @click="handleSubmit(update)" />
       </div>
       <div v-if="step===2">
         <btn v-tooltip="$t('btn.back')" :label="$t('btn.back')" variant="tertiary" @click="back()"/>
         <btn-save :label="$t('btn.continue')"
                   :disabled="!changed || loading"
                   @click="handleSubmit(update)" />
       </div>
...
<script>
  ...
  import { Btn, FormSection, BtnSave, BtnDelete, BtnCancel } from '@/components/generic/form';
  ...
  export default {
    ...
    computed: {
      ...mapState(['activeObjective', 'activeItem', 'activeItemRef', 'keyResults', 'organizations', 'departments', 'products']),
      isOrganization() {
      ...
      changed() {
        ...
        const descriptionEdited = this.keyResult?.description !== this.data?.keyResult?.description;
        const ownerEdited = this.keyResult?.owner !== this.data?.keyResult?.owner;

        if (nameEdited || descriptionEdited || ownerEdited) {
          return true;
        }
      },
      ownerOptions() {
        ...
        if (this.isOrganization) {
          thisLevel = this.organizations.find(o => o.id === this.activeItem.id);
          children = this.departments.filter(
            (department) => department.organization.id === this.activeItem.id
          );
        } else if (this.isDepartment) {
          thisLevel = this.departments.find(d => d.id === this.activeItem.id);
          children = this.products.filter(
            (product) => product.department.id === this.activeItem.id
          );
        }

        let thisLevelOption = {
          value: thisLevel.path,
          name: this.activeItem.name,
        };

        // Set currentLevel as default option for key result owner
        if (!this.keyResult.id) {
          this.keyResult.parent = thisLevelOption;
        }

        let childrenOptions = children.map((child) => ({
          value: child.path,
          name: child.name,
        }));

        return [thisLevelOption, ...childrenOptions];
      },
    },
  ...
  methods: {
    ...
    async update() {
      if (this.step === 1) {
        ...
        this.loading = true;
        try {
          const { id, name, description, unit, weight, startValue, targetValue} = this.keyResult;
